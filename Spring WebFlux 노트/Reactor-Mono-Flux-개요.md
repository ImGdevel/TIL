# Reactor Mono / Flux 개요

> 태그: `#webflux` `#reactor` `#mono` `#flux`<br>
> 작성일: 2026-06-25<br>
> 최종 수정일: 2026-06-25

## 정의

Reactor는 리액티브 스트림즈 명세를 구현한 Spring WebFlux의 공식 라이브러리로, `Publisher`/`Subscriber`/`Subscription`을 직접 구현하지 않고도 0~1개 요소를 다루는 `Mono`와 0개 이상 요소를 다루는 `Flux`로 비동기 데이터 흐름을 선언적으로 제어할 수 있게 한다.
...
- 이로 인해 비동기 및 스트림 로직을 보다 직관적으로 사용하고, 복잡도를 낮추며, 전체 데이터 흐름을 사용자 관점에서 제어할 수 있게 된다.

> "이제 이 리액터로 넘어오면서 그것들이 어느 정도 다 구현이 되어 있는 상태에서 우리는 사용하는 입장, 사용자의 입장으로 점점 넘어온다고 봐 주시면 될 거 같습니다." (원본 TIL 강의 인용)
...
> "콜드 퍼블리셔는 특정 값을 이미 정해두고 보내고, 다 보내고 나서 컴플리트하고 끝내는 퍼블리셔." (원본 TIL 강의 인용)
...
> "서브스크라이브 하게 되면 계속 내가 받고 있는 값들을 넘겨 주는 구조, 이게 핫 퍼블리셔." (원본 TIL 강의 인용)
...
해당 없음 (Mono/Flux는 타입 선택의 문제이며, 콜드/핫 선택에 따른 실질적 트레이드오프는 [Reactor-백프레셔](Reactor-백프레셔.md)의 3.2절 참고)
...
- 원본 TIL 노트가 강의 인용을 포함하나 1차 출처(공식 문서)는 명시되어 있지 않음. 필요 시 [Project Reactor 공식 레퍼런스](https://projectreactor.io/docs/core/release/reference/)에서 `Mono`/`Flux` API를 직접 확인.
