...
> 태그: `#concurrency` `#thread-pool` `#java` `#spring`<br>
> 작성일: 2026-06-25<br>
> 최종 수정일: 2026-06-25

## 정의

쓰레드 풀은 미리 만들어둔 쓰레드 여러 개를 재사용하면서 작업(Runnable/Callable)을 실행하는 구조로, 매 요청마다 새 쓰레드를 만들지 않고 풀에서 빌려 쓰고 다시 돌려준다. `corePoolSize`/`maximumPoolSize`/`workQueue`/`rejectedExecutionHandler` 등의 파라미터 조합과 CPU 바운드/I/O 바운드 작업 특성에 따라 적정 크기가 달라진다.
...
쓰레드 풀은 "얼마나 많이 만들 것인가?"를 잘못 잡으면, CPU는 100%인데 처리량은 오히려 낮아지는 상황이 온다.
...
- 쓰레드가 대부분 I/O 대기로 놀고 있기 때문에, CPU 코어 수보다 더 많은 쓰레드를 써도 된다.
...
쓰레드 풀은 "만들어 놓고 끝"이 아니라, **지표를 보면서 조정해야 제 역할을 한다.**
...
> "쓰레드 풀을 몇 개, 몇 개의 쓰레드로 만들까?"보다 "어떤 종류의 일을 어떤 풀에 맡길까?"를 먼저 정하는 게 훨씬 중요하다.
...
를 한 번 점검해 보면 좋다. 그 위에서 조금씩 풀/큐/정책을 조정해 나가면, "감으로 쓰던 동시성"이 조금씩 더 통제 가능한 상태에 가까워진다.

## 트레이드오프

규모 가정: 단일 JVM 프로세스 내부의 쓰레드 풀 설정 기준. 분산된 여러 인스턴스에 걸친 동시성 제어(예: 분산 락, 글로벌 레이트 리밋)는 별도 주제다.

| 항목 | 내용 |
|---|---|
| 일관성 | 해당 없음 (단일 프로세스 내 쓰레드 풀 설정이며 분산 일관성과 무관) |
| 가용성 | 무한 큐 + 고정 쓰레드 풀 조합은 느린 작업 누적 시 OOM으로 전체 서비스 가용성에 영향 — 유한 큐 + 거부 정책으로 장애를 국소화해야 함 (특징/상세 4, 7-1번) |
| 지연 | CPU 바운드는 코어 수 근접 쓰레드 수가 컨텍스트 스위칭 오버헤드를 줄여 지연에 유리. I/O 바운드는 코어 수보다 많은 쓰레드가 처리량에 유리하나, 무작정 늘리면 캐시 미스/컨텍스트 스위칭으로 오히려 악화 (특징/상세 3, 7-5번) |
| 비용 | 쓰레드 수가 늘어날수록 스택 메모리(쓰레드당 기본 스택 크기) 비용 증가. 풀을 역할별로 분리하면(CPU/IO) 관리 비용은 늘지만 상호 간섭으로 인한 처리량 손실을 줄임 |
| 운영부담 | `Executors.newXXX` 기본 헬퍼 사용 시 함정(무제한 큐/쓰레드)을 직접 설정으로 회피해야 함. 모니터링 지표(active thread, queue size, rejected count) 없이는 튜닝이 감으로만 이루어짐 — Micrometer 연동 권장 (특징/상세 6번) |

## 실무 경험

해당 없음 (관련 실무 내용은 특징/상세 3, 4, 7번 "쓰레드 풀 사이즈 전략", "큐와 거부 정책", "자주 하는 실수와 주의점" 참고)

## 참고

- 원본 학습 노트(TIL)에 `> 연결된 정리본: 비동기 전략과 쓰레드 풀 관리` 링크(`../../../TIL.wiki/async-strategy-and-thread-pool-management.md`)가 있었으나, 이 볼트 외부의 위키 경로라 마크다운 링크로 옮기지 않음. 필요 시 원본 TIL 위키에서 직접 확인.
