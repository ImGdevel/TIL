...
> 태그: `#security` `#spring-security` `#filter-chain` `#architecture`<br>
> 작성일: 2026-06-24<br>
> 최종 수정일: 2026-06-24

## 정의

Spring Security는 `DelegatingFilterProxy`(서블릿 필터-Spring Bean 연결) → `FilterChainProxy`(중앙 진입점, 체인 선택) → `SecurityFilterChain`(URL 패턴별 필터 조합) → 개별 Security Filters로 이어지는 계층 구조로 동작하며, 필터의 실행 순서 자체가 보안 정확성을 결정한다.
...
해당 없음. 다만 6장 "필터 배치 위치 가이드" 표가 필터 타입별 배치 위치와 이유를 비교한 내용이므로 실질적인 트레이드오프 비교에 해당한다.

## 실무 경험

해당 없음 (관련 실무 내용은 특징/상세 6장 "실무 관점의 활용법"에 포함).

## 참고

원본 학습 노트(TIL)에 `> 연결된 정리본: Servlet 아키텍처` 링크(`../../../TIL.wiki/spring-security-servlet-architecture.md`)가 있었으나, 이 볼트 외부의 위키 경로라 마크다운 링크로 옮기지 않음. 필요 시 원본 TIL 위키에서 직접 확인.
