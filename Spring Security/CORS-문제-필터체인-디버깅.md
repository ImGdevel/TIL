...
> 태그: `#security` `#spring-security` `#cors` `#debugging` `#filter-chain`<br>
> 작성일: 2026-06-24<br>
> 최종 수정일: 2026-06-24

## 정의

"분명 CORS 설정을 했는데도 왜 여전히 CORS 에러가 날까?"라는 문제는 Preflight(OPTIONS) 요청이 서버까지 도달하는지, Spring Security 필터 체인의 어느 필터가 이 요청을 막는지, `http.cors()` + `CorsConfigurationSource` 설정이 실제 요청 스펙과 일치하는지를 필터 체인을 따라 순서대로 확인하면 추적 가능한 버그로 바뀐다.
...
- `> 연결된 정리본: CORS 원인 추적` → `../../../TIL.wiki/spring-security-cors-troubleshooting.md`
- `> 연결된 정리본: CORS` → `../../../TIL.wiki/spring-security-cors.md`
