...
> 태그: `#security` `#spring-security` `#filter-chain` `#jwt`<br>
> 작성일: 2026-06-24<br>
> 최종 수정일: 2026-06-24

## 정의

Spring Security는 서블릿 필터 체인 위에서 동작하며, 커스텀 필터(예: JWT 인증 필터)는 "인증 정보가 필요한가", "인증을 직접 수행하는가", "모든 요청에 적용되는가"라는 세 질문을 기준으로 `addFilterBefore`/`addFilterAfter`/`addFilterAt`을 이용해 기존 필터 대비 정확한 위치에 배치해야 한다.
...
### 원본 노트에서 다루지 않은 내용 / 다음 학습 추천

원본 학습 노트(TIL)가 명시적으로 범위 밖이라고 밝힌 내용과, 다음 학습으로 추천한 주제를 누락 없이 기록한다.
...
해당 없음. 다만 "주요 필터의 역할" 표가 필터별 역할/실행 시점 비교에 해당한다.

## 실무 경험

해당 없음 (관련 실무 내용은 특징/상세의 "실전 구현: JWT 인증 필터", "검증", "심화: 필터 체인 디버깅"에 포함).

## 참고

원본 학습 노트(TIL)에 다음 링크들이 있었으나, 이 볼트 외부의 위키 경로라 마크다운 링크로 옮기지 않음. 필요 시 원본 TIL 위키에서 직접 확인.

- `> 연결된 정리본: Servlet 아키텍처` → `../../../TIL.wiki/spring-security-servlet-architecture.md`
- `> 연결된 정리본: 커스텀 필터 설계` → `../../../TIL.wiki/spring-security-custom-filter-design.md`
- `> 연결된 정리본: JWT 필터 배치` → `../../../TIL.wiki/spring-security-jwt-filter-positioning.md`
- `> 연결된 정리본: Filter Chain 예외 처리` → `../../../TIL.wiki/spring-security-filter-chain-exception-handling.md`
...
- [Spring-Security-아키텍처-이해하기](Spring-Security-아키텍처-이해하기.md)
- [Spring-Security-인증-아키텍처-이해하기](Spring-Security-인증-아키텍처-이해하기.md)
