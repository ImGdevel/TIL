...
> 태그: `#security` `#spring-security` `#authentication` `#architecture`<br>
> 작성일: 2026-06-24<br>
> 최종 수정일: 2026-06-24

## 정의

Spring Security의 인증은 Filter가 추출한 자격 증명을 `AuthenticationManager`(실무에서는 `ProviderManager`)가 여러 `AuthenticationProvider`에게 위임하고, Provider가 `UserDetailsService`로 사용자 정보를 조회·검증한 뒤 인증된 `Authentication` 객체를 `SecurityContextHolder`에 저장하는 협력 구조로 동작한다.
...
이 설정을 적용하면 기존 폼 로그인 인증과 JWT 인증이 동시에 동작합니다.
...
각 컴포넌트는 단일 책임 원칙을 따르며, 인터페이스 기반 설계로 확장성을 보장합니다. 커스텀 인증 로직을 추가해야 하는 경우에는 AuthenticationProvider를 구현하거나 필터를 추가하는 방식으로 확장할 수 있습니다.

핵심은 **각 컴포넌트의 역할과 책임을 명확히 이해하는 것**입니다. 이를 전제로 하면, 복잡한 인증 요구사항도 Spring Security의 아키텍처 안에서 일관된 방식으로 설계할 수 있습니다.

## 트레이드오프

해당 없음. 다만 9장 "AuthenticationEntryPoint 사용 시나리오" 표가 시나리오별 구현체 선택 비교에 해당한다.

## 실무 경험

해당 없음 (관련 실무 내용은 특징/상세 12장 "실무 적용: 커스텀 인증 구현", 13장 "실무 관점의 주의사항"에 포함).

## 참고

원본 학습 노트(TIL)에 `> 연결된 정리본: 인증 아키텍처` 링크(`../../../TIL.wiki/spring-security-authentication-architecture.md`)가 있었으나, 이 볼트 외부의 위키 경로라 마크다운 링크로 옮기지 않음. 필요 시 원본 TIL 위키에서 직접 확인.
