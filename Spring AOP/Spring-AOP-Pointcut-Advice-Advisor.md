...
> 태그: `#spring` `#aop` `#pointcut` `#advice` `#advisor`<br>
> 작성일: 2026-06-25<br>
> 최종 수정일: 2026-06-25

## 정의

Spring AOP(Aspect-Oriented Programming)는 횡단 관심사(Cross-Cutting Concerns)를 효과적으로 분리하고 관리하기 위한 프로그래밍 패러다임이다. 핵심은 3가지 요소로 구성된다: Pointcut(어디에 적용할지), Advice(무엇을 할지), Advisor(Pointcut과 Advice의 조합 — AOP 적용의 완전한 단위).
...
| 항목 | 내용 |
|---|---|
| 일관성 | 해당 없음 (AOP는 횡단 관심사 분리 기법이며 분산 시스템 일관성과 직접적 연관 없음) |
| 가용성 | 해당 없음 |
| 지연 | Pointcut 매칭은 프록시 호출마다 발생 — `execution(* *(..))`처럼 광범위한 표현식은 매칭 비용이 커지므로 구체적인 패키지/타입으로 범위를 좁혀야 함 (특징/상세 1.4 "Pointcut 성능 최적화" 참고). `@Around`에서 `proceed()` 호출 안에서 블로킹 I/O를 수행하면 그만큼 지연이 그대로 노출됨 |
| 비용 | CGLIB/JDK 동적 프록시 생성 비용, Aspect 수가 많아질수록 프록시 체인 깊이 증가로 인한 호출 오버헤드 |
| 운영부담 | 여러 Aspect 간 실행 순서(`@Order`)를 명시하지 않으면 의도하지 않은 순서로 적용될 수 있어 디버깅이 어려움. Self-invocation(같은 클래스 내부 메서드 호출 시 프록시를 거치지 않아 Advice가 적용되지 않는 문제)이 가장 흔한 실무 트러블슈팅 포인트 — 특징/상세 5번 "실무에서 주의할 점" 참고 |

## 실무 경험

해당 없음 (관련 실무 내용은 특징/상세 1.4, 2.4, 3.4번 "실무 팁", 5번 "실무에서 주의할 점", 6번 "Best Practices", 7번 "FAQ" 참고)

## 참고

- 원본 학습 노트(TIL)에 `> 연결된 정리본: Spring AOP 핵심 개념` 링크(`../../../TIL.wiki/spring-aop-pointcut-advice-advisor.md`)가 있었으나, 이 볼트 외부의 위키 경로라 마크다운 링크로 옮기지 않음. 필요 시 원본 TIL 위키에서 직접 확인.
### 공식 문서
