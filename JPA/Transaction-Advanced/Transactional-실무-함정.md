...
> 태그: `#jpa` `#transactional` `#spring-aop` `#rollback`<br>
> 작성일: 2026-06-24<br>
> 최종 수정일: 2026-06-24

## 정의

`@Transactional`은 프록시 기반 AOP로 동작하므로 self-invocation과 private 메서드에는 적용되지 않고, 기본 롤백 규칙(`RuntimeException`/`Error`만 롤백)과 rollback-only 전파, final 클래스/메서드 제약, 테스트의 자동 롤백을 정확히 이해해야 운영 코드에서 의도대로 동작한다.
