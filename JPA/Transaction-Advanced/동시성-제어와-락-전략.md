...
> 태그: `#jpa` `#concurrency` `#lock` `#optimistic-lock` `#pessimistic-lock`<br>
> 작성일: 2026-06-24<br>
> 최종 수정일: 2026-06-24

## 정의

JPA 동시성 제어는 격리 수준만으로 풀리지 않으며, Lost Update/Write Skew/중복 처리 같은 문제 유형에 따라 낙관적 락·비관적 락·조건부 update·unique constraint를 조합해 선택한다.
...
해당 없음 — 재고 차감 전략별 장단점/추천 상황은 위 5번 비교표 참고.
