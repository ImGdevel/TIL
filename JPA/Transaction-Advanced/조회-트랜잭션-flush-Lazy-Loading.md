...
> 태그: `#jpa` `#transaction` `#flush` `#lazy-loading` `#osiv`<br>
> 작성일: 2026-06-24<br>
> 최종 수정일: 2026-06-24

## 정의

조회 트랜잭션은 `readOnly = true`로 변경 의도가 없음을 명시하고, flush는 커밋과 별개로 쿼리 실행 전·명시적 호출 시에도 발생할 수 있다는 점을 인지하면서, Lazy Loading 예외는 트랜잭션을 늘리는 대신 서비스 계층의 명시적 조회(fetch join/DTO projection)로 해결한다.
...
- [JPA-조회-성능-최적화](JPA-조회-성능-최적화.md)
- [트랜잭션-경계와-전파](트랜잭션-경계와-전파.md)
- [Transactional-실무-함정](Transactional-실무-함정.md)
