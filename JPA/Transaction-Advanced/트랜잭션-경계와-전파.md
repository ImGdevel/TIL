...
> 태그: `#jpa` `#transaction` `#propagation` `#spring`<br>
> 작성일: 2026-06-24<br>
> 최종 수정일: 2026-06-24

## 정의

트랜잭션 경계는 "하나의 비즈니스 불변식을 지키는 최소 범위"로 서비스 유스케이스 메서드에 두며, 전파 전략(`REQUIRED`/`REQUIRES_NEW`/`NESTED`/`SUPPORTS`/`MANDATORY`/`NOT_SUPPORTED`)은 그 경계 안에서 호출되는 메서드가 기존 트랜잭션에 참여할지, 독립된 트랜잭션을 새로 시작할지를 결정한다.

## 특징 / 상세

> Spring `@Transactional`과 JPA/Hibernate를 기준으로, 실무에서 자주 만나는 트랜잭션 문제와 해결 전략을 정리한 시리즈(트랜잭션 경계/전파, 동시성·락, 외부 시스템 일관성, 조회/flush/Lazy Loading, 배치·데드락·긴 트랜잭션, `@Transactional` 실무 함정, 안티패턴 체크리스트) 중 첫 번째 문서.
...
해당 없음 — 전파 전략별 특징/사용처는 위 6번 표 참고.
...
- [외부시스템-트랜잭션-일관성](외부시스템-트랜잭션-일관성.md)
