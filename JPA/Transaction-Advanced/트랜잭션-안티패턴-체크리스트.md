...
> 태그: `#jpa` `#transaction` `#antipattern` `#checklist`<br>
> 작성일: 2026-06-24<br>
> 최종 수정일: 2026-06-24

## 정의

트랜잭션 내부 외부 API 호출, exists 검사 후 save, 조회-검사-update, 과도하게 넓은 트랜잭션 범위, REQUIRES_NEW의 부분 성공 오남용, 트랜잭션 내부 이벤트 발행, 무심한 Dirty Checking, flush·commit 혼동, 캐시-트랜잭션 원자성 오해, 동시성 테스트 누락이라는 10가지 실무 안티패턴을 모아 정리한 체크리스트.
