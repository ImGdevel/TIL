...
> 태그: `#jpa` `#n+1` `#fetch-join` `#querydsl` `#performance`<br>
> 작성일: 2026-06-24<br>
> 최종 수정일: 2026-06-24

## 정의

JPA 조회 성능 최적화는 모든 연관관계를 LAZY로 시작한 뒤 단건 조회는 Fetch Join, 목록/페이징 조회는 QueryDSL Constructor Projection으로 분리하고, FK만 필요하면 `getReferenceById()`·카운터 증감은 Bulk Update·깜빡한 N+1은 전역 `default_batch_fetch_size`로 방어하는, "쿼리 개수를 예측 가능하게 만드는" 일련의 패턴을 따르는 것이다.
...
이 문서는 실무에서 겪은 성능 이슈와 그 해결 과정을 정리한 것이다. 주요 니즈는:
...
**단점**: 컬렉션 Fetch Join은 페이징이 깨진다 (3번 항목에서 설명)
...
**모든 연관관계는 LAZY**: 불필요한 조인을 방지하고, 조회를 명시적으로 제어한다.

**단건은 Fetch Join, 목록은 Constructor Projection**: 각각의 사용 시점이 명확하다. 혼동하지 말자.

**QueryDSL로 동적 쿼리**: JPQL 문자열 조작은 버그의 온상. QueryDSL로 타입 안전하게.

**Entity 로딩 최소화**: FK만 필요하면 getReferenceById, 카운터 수정은 Bulk Update.

**Batch Fetch Size는 필수**: 깜빡한 N+1을 막아주는 최후의 보루. 반드시 설정.
...
해당 없음 — 시나리오별 전략 선택은 위 6번 가이드 표 참고.
...
원본 학습 노트(TIL)에 두 개의 `> 연결된 정리본` 링크(`../../../TIL.wiki/jpa-query-and-cache-strategies.md`, `../../../TIL.wiki/fetch-join-vs-entitygraph-vs-batch-size.md`)가 있었으나, 이 볼트 외부의 위키 경로라 마크다운 링크로 옮기지 않음. 필요 시 원본 TIL 위키에서 직접 확인.

## 관련 내용

- [일대다-관계-설계-전략](일대다-관계-설계-전략.md)
- [조회-트랜잭션-flush-Lazy-Loading](조회-트랜잭션-flush-Lazy-Loading.md)
