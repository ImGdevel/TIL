...
> 태그: `#resilience` `#circuit-breaker` `#distributed-systems` `#redis`<br>
> 작성일: 2026-06-25<br>
> 최종 수정일: 2026-06-25

## 정의

Resilience4j 서킷 브레이커는 인스턴스마다 독립적인 상태를 갖기 때문에, 여러 인스턴스로 스케일아웃된 환경에서는 한 인스턴스가 OPEN 되어도 다른 인스턴스가 이를 모른 채 트래픽을 몰아 받아 연쇄 장애로 이어질 수 있다. 이를 막으려면 Redis Pub/Sub 등으로 상태 전환 시점에만 동기화하거나, 서비스 메시로 인프라 레이어에서 처리하거나, 현실적 타협(작은 슬라이딩 윈도우 + 모니터링)을 택한다.
...
규모 가정: 동일 서비스가 여러 인스턴스로 스케일아웃된 환경(로드밸런서 뒤 N개 인스턴스) 기준. 단일 인스턴스라면 이 문서의 동기화 문제 자체가 발생하지 않는다 — [서킷-브레이커-Resilience4j](서킷-브레이커-Resilience4j.md) 참고.

| 항목 | 내용 |
|---|---|
| 일관성 | Redis Pub/Sub 방식은 상태 전환 "시점"에만 동기화하는 최종 일관성 — 전파 지연 동안 인스턴스 간 상태 불일치 가능 (특징/상세 3-1, 7번) |
| 가용성 | Redis 장애 시에도 로컬 CircuitBreaker는 독립 동작을 유지해 동기화 기능만 저하되고 서킷 브레이커 자체 가용성은 보존 (특징/상세 3-1, 5번 "Redis가 죽으면?") |
| 지연 | 상태 전파는 Pub/Sub 발행~구독 경로의 네트워크 지연만 추가 — 로컬 판단 경로(실패율 계산 등) 자체의 지연에는 영향 없음 |
| 비용 | Redis 인스턴스 운영 비용(Pub/Sub 전용이라도 고가용성 구성 필요). 서비스 메시 대안은 Istio/Linkerd 운영 비용이 더 큼 (특징/상세 7번 비교표) |
| 운영부담 | `volatile` 플래그 기반 장애 감지/복구 로직, 신규 인스턴스 초기 상태 동기화(`@PostConstruct`), 5초 주기 헬스체크 등 추가 컴포넌트를 직접 운영해야 함 (특징/상세 4, 6번) |

## 실무 경험

해당 없음 (관련 실무 내용은 특징/상세 4번 "Redis Pub/Sub 구현"의 설계 결정과 6번 "설계 결정 요약" 참고)
