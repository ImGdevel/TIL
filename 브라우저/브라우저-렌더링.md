...
> 태그: `#browser` `#rendering` `#virtual-dom`<br>
> 작성일: 2026-06-25<br>
> 최종 수정일: 2026-06-25

## 정의

브라우저 렌더링은 HTML·CSS·JS를 입력으로 받아 DOM 트리 생성 → CSSOM 트리 생성 → Render Tree 생성 → Layout(위치/크기 계산) → Paint(픽셀로 그리기) 순서로 화면에 픽셀을 그려내는 과정이다.
...
해당 없음 — 브라우저 렌더링 파이프라인과 Virtual DOM의 동작 원리에 대한 개념 정리이며 분산 시스템 트레이드오프(일관성/가용성/지연/비용/운영부담)와는 무관. 런타임 Diffing vs 컴파일타임 결정에 따른 성능/번들 크기 영향은 위 특징/상세의 "Virtual DOM의 진짜 장점", "React vs Svelte 비교" 항목 참고.

## 실무 경험

해당 없음 (관련 실무 내용은 특징/상세의 "JavaScript와 Reflow/Repaint" — Reflow 최소화를 위한 DocumentFragment 사용 패턴 — 참고)
...
- [Event-Loop와-비동기-처리](../javascript/Event-Loop와-비동기-처리.md)
