# Jackson 자주 발생하는 문제와 해결법

> 태그: `#java` `#jackson` `#json` `#troubleshooting`<br>
> 작성일: 2026-06-25<br>
> 최종 수정일: 2026-06-25

## 정의

Jackson 직렬화/역직렬화 과정에서 반복적으로 마주치는 8가지 문제 유형(순환참조, 날짜 포맷, Unknown Property, 네이밍 컨벤션, Enum 처리, 컬렉션/제네릭 역직렬화, 필드 노출 제어, 성능)과 각각의 증상/원인/해결법을 정리한 문제 해결 레퍼런스다.
...
해당 없음 — 라이브러리 트러블슈팅 레퍼런스이며 분산 시스템 트레이드오프(일관성/가용성/지연/비용/운영부담)와는 무관. 성능/메모리 관련 트레이드오프는 위 특징/상세 2.8번 "성능 및 메모리 문제" 참고.

## 실무 경험

해당 없음 (관련 실무 내용은 특징/상세 3번 "Jackson을 사용할 때의 기본 팁" 참고)
...
- [Jackson-JSON-처리](Jackson-JSON-처리.md)
