...
> 태그: `#java` `#jackson` `#json`<br>
> 작성일: 2025-11-03<br>
> 최종 수정일: 2026-06-25

## 정의

Jackson은 Java 객체와 JSON 문자열 사이를 변환하는 직렬화(Serialization, 객체→JSON)/역직렬화(Deserialization, JSON→객체) 라이브러리로, Spring Boot에 기본 내장되어 Java 생태계에서 JSON 처리의 사실상 표준으로 쓰인다.
...
해당 없음 — 라이브러리 사용법에 대한 개념 정리이며 분산 시스템 트레이드오프(일관성/가용성/지연/비용/운영부담)와는 무관. ObjectMapper 재사용 여부에 따른 성능 영향은 위 특징/상세 "실무에서 알아두면 좋은 것들" 1번, "성능 최적화 팁" 참고.

## 실무 경험

해당 없음 (관련 실무 내용은 특징/상세 "실무에서 알아두면 좋은 것들", "자주 하는 실수와 해결법", "성능 최적화 팁" 참고)

## 참고

- 원본 학습 노트(TIL)에 `> 연결된 정리본: Jackson과 JSON 처리` 링크(`../../../TIL.wiki/jackson-json-processing.md`)가 있었으나, 이 볼트 외부의 위키 경로라 마크다운 링크로 옮기지 않음. 필요 시 원본 TIL 위키에서 직접 확인.
...
- [Jackson-문제와-해결](Jackson-문제와-해결.md)
