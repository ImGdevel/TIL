...
> 태그: `#network` `#tls` `#https` `#security` `#handshake`<br>
> 작성일: 2026-06-23<br>
> 최종 수정일: 2026-06-23

## 정의

TLS(Transport Layer Security)는 TCP 연결 위에서 동작하는 보안 프로토콜(HTTPS = HTTP + TLS)로, 핸드셰이크 과정에서 인증서로 서버를 인증하고 키 교환 방식(RSA 또는 ECDHE)으로 대칭키를 합의하며, ECDHE는 Forward Secrecy를 제공해 장기 개인키가 노출되어도 과거 세션이 안전하다는 점에서 RSA 방식과 갈린다.
...
| 항목 | 내용 |
|---|---|
| 일관성 | 해당 없음 — 키 교환 방식은 세션 단위 보안 속성이라 데이터 일관성과는 무관 |
| 가용성 | 둘 다 핸드셰이크 실패 시 연결 불가, 가용성 차이는 거의 없음 |
| 지연 | ECDHE가 연산량이 더 많지만 현대 CPU 기준 체감 차이는 작음 |
| 비용 | 동일 인증서/인프라로 둘 다 처리 가능, 추가 비용 차이는 미미 |
| 운영부담 | ECDHE는 Forward Secrecy 때문에 기본값으로 권장되며, RSA 단독 방식은 TLS 1.3에서 제거되어 레거시 호환 목적 외에는 점차 배제하는 방향 |
...
- [리버스-프록시](리버스-프록시.md)
- [http1.1-http2-keepalive-비교](http1.1-http2-keepalive-비교.md)
