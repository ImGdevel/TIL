# https://google.com 을 치면 일어나는 일 (웹 요청 전체 흐름)

> 태그: `#network` `#overview` `#dns` `#tcp` `#tls` `#http` `#was`<br>
> 작성일: 2026-06-23<br>
> 최종 수정일: 2026-06-23

## 정의

브라우저 주소창에 URL을 입력하고 엔터를 누르는 순간부터 화면이 뜨기까지는 DNS 조회 → TCP 연결 → TLS 핸드셰이크 → HTTP 요청 → 리버스 프록시 → WAS → DB → 응답의 8단계를 거치며, 각 단계는 독립된 네트워크/인프라 개념이 순서대로 맞물려 동작한다.
...
→ 자세한 내용: [TCP-3-way-핸드셰이크](TCP-3-way-핸드셰이크.md)
...
해당 없음 — 개별 단계의 트레이드오프는 각 단계의 상세 노트 참고.
