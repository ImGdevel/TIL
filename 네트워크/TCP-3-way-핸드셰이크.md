...
> 태그: `#network` `#tcp` `#handshake` `#security`<br>
> 작성일: 2026-06-23<br>
> 최종 수정일: 2026-06-23

## 정의

TCP(Transmission Control Protocol)는 신뢰성 있는 연결을 보장하는 연결 지향형 프로토콜로, 데이터를 보내기 전에 SYN→SYN-ACK→ACK 3단계로 연결을 맺어 양방향 모두가 서로의 초기 순서번호(ISN)를 확인하며, 이 과정의 구조적 약점이 SYN Flood 같은 공격으로 악용될 수 있다.
...
> 자세한 4-way close 절차와 TIME_WAIT은 [TCP-4-way-종료](TCP-4-way-종료.md), [TIME-WAIT](TIME-WAIT.md) 참고.
