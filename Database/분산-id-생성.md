...
> 태그: `#db` `#distributed-systems` `#id-generation` `#sharding`<br>
> 작성일: 2026-06-23<br>
> 최종 수정일: 2026-06-23

## 정의

분산 환경에서는 여러 노드가 동시에 ID를 생성해도 전역적으로 유일해야 하며, UUID/Snowflake/ULID/AUTO_INCREMENT 오프셋 전략 중 정렬 가능성·인덱스 성능·구현 복잡도의 트레이드오프에 따라 선택한다.
...
| 항목 | 내용 |
|---|---|
| 일관성 | 해당 없음 — ID 유일성은 모든 전략이 보장, 일관성 모델과 무관 |
| 가용성 | 모든 전략이 중앙 서버 없이 노드 독립적으로 생성 가능 (AUTO_INCREMENT 제외) |
| 지연 | Snowflake/UUID는 로컬 생성으로 지연 거의 없음 |
| 비용 | Snowflake는 8byte로 UUID/ULID(16byte) 대비 저장 공간 절반 |
| 운영부담 | Snowflake는 머신 ID 관리·시계 동기화 필요, UUID/ULID는 운영 부담 거의 없음 |
