...
NoSQL(Not Only SQL)은 RDB의 고정 스키마·수직 확장 한계를 극복하기 위해 등장한 비관계형 데이터베이스 군으로, CAP/BASE 트레이드오프를 바탕으로 Key-Value, Document, Wide-Column, Graph 네 가지 유형으로 나뉜다.
...
유스케이스: 세션 저장(로그인 토큰), 캐시(DB 앞단, 자주 읽는 데이터), 장바구니, 실시간 랭킹/카운터.
...
유스케이스: 상품 카탈로그(상품마다 속성이 다름), 콘텐츠 관리(블로그, 뉴스), 사용자 프로필, 로그 수집.

세부 내용은 [nosql-도큐먼트-db](nosql-도큐먼트-db.md) 참고.
...
**ScyllaDB vs Cassandra**: ScyllaDB는 Cassandra를 Java에서 C++로 재구현한 DB다. JVM GC 오버헤드를 없애고 CPU 코어를 직접 제어해서 동일 하드웨어에서 Cassandra 대비 수 배 ~ 수십 배 성능이 나온다. API가 완전히 호환되어 Cassandra를 쓰던 곳에서 드롭인 교체가 가능하다.

유스케이스: IoT 센서 데이터(초당 수만 건 쓰기), 시계열 메트릭, 대규모 활동 피드/타임라인, 로그/이벤트 수집.

> Netflix는 실제로 Cassandra를 사용한다. 초당 수백만 건의 쓰기를 감당하기 위해서다.

세부 내용은 [nosql-와이드컬럼](nosql-와이드컬럼.md) 참고.
...
유스케이스: SNS 친구 추천("친구의 친구"), 추천 시스템, 사기 탐지(계좌 간 관계 패턴 분석), 권한/역할 계층 관리.

세부 내용은 [nosql-그래프-db](nosql-그래프-db.md) 참고.
...
| 항목 | 내용 |
|---|---|
| 일관성 | CP 선택 시 강한 일관성, AP 선택 시 Eventually Consistent |
| 가용성 | AP 선택 시 네트워크 장애에도 응답, CP 선택 시 장애 구간 응답 중단 가능 |
| 지연 | AP가 일반적으로 더 낮은 지연 (즉시 동기화 불필요) |
| 비용 | 유형/벤더에 따라 다름 — 해당 없음(일반화 불가) |
| 운영부담 | AP는 충돌 해소·일관성 수렴 로직 운영 부담 추가 |
