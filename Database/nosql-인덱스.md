...
> 태그: `#db` `#nosql` `#mongodb` `#index`<br>
> 작성일: 2026-06-23<br>
> 최종 수정일: 2026-06-23

## 정의

NoSQL 인덱스는 스키마가 없는 구조에서 COLLSCAN(전체 컬렉션 탐색)을 피하기 위한 필수 장치로, 중첩 필드·배열(Multikey)·복합(ESR 규칙)·부분·Sparse 인덱스 등 RDB보다 다양한 형태를 지원하지만 많을수록 쓰기 성능을 깎는 트레이드오프가 있다.
...
| 항목 | 내용 |
|---|---|
| 일관성 | 해당 없음 — 인덱스는 일관성 모델과 무관 |
| 가용성 | 해당 없음 |
| 지연 | 인덱스 많을수록 읽기 지연↓, 쓰기 지연↑ |
| 비용 | 인덱스 크기만큼 스토리지/메모리 비용 증가 |
| 운영부담 | 부분/Sparse 인덱스로 크기 최소화, ESR 규칙 준수 필요 — 설계 실수 시 COLLSCAN으로 빠짐 |
