...
> 태그: `#db` `#connection-pool` `#hikaricp` `#spring`<br>
> 작성일: 2026-06-23<br>
> 최종 수정일: 2026-06-23

## 정의

DB 커넥션은 생성/해제 비용이 크고 DB가 동시에 처리할 수 있는 개수가 제한적인 비싼 리소스이므로, Connection Pool(HikariCP 등)로 미리 만들어둔 커넥션을 재사용하며 풀 크기·타임아웃·누수 감지를 DB/애플리케이션 스펙에 맞게 관리해야 한다.
...
- 불필요하게 넓은 범위에 `@Transactional`을 걸어두면, 그 시간 동안 커넥션을 다른 작업이 사용할 수 없다.
...
DB 커넥션 문제를 겪을 때 처음 떠오르는 해결책은 "풀 크기를 늘리자"일 때가 많다. 하지만 실제로는 그 전에 볼 게 많다.
...
| 항목 | 내용 |
|---|---|
| 일관성 | 해당 없음 — 커넥션 풀은 일관성 모델과 무관 |
| 가용성 | 풀 고갈 시 신규 요청이 타임아웃으로 실패 — 풀 크기/타임아웃 설정이 가용성에 직결 |
| 지연 | 풀 크기 부족 시 대기 큐 지연 증가, 과도하게 크면 DB 측 컨텍스트 스위칭 비용 증가 |
| 비용 | 풀 크기는 곧 DB 서버 리소스(메모리, 커넥션 슬롯) 점유 비용 |
| 운영부담 | 누수 감지·모니터링 지표 설정·인스턴스 수 변경 시마다 풀 크기 재조정 필요 |
...
- 원본에 로컬 위키 경로(`../../../TIL.wiki/database-connection-and-transaction-management.md`)로의 연결된 정리본 참조가 있었으나, 이 볼트 외부 경로라 링크로 옮기지 않음. 필요 시 원본 TIL 위키에서 직접 확인.
