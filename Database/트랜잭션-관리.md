...
> 태그: `#db` `#transaction` `#spring` `#jpa`<br>
> 작성일: 2026-06-23<br>
> 최종 수정일: 2026-06-23

## 정의

Spring/JPA 트랜잭션 관리는 ACID를 보장하는 작업 단위를 서비스 계층의 `@Transactional` 경계로 선언하는 것이 기본이며, self-invocation·readOnly 오용·Lazy 로딩 트랜잭션 범위 이탈 같은 실수를 피해야 한다.
...
JPA/Spring에서 트랜잭션 관리는 대부분 **서비스 계층**에서 이루어지며, Spring이 프록시/프레임워크 레벨에서 커넥션과 커밋/롤백을 대신 관리한다.

### Spring에서 트랜잭션을 관리하는 방법
...
#### XML/Java Config 기반 AOP 설정 (요즘은 거의 사용 X)

- 과거에는 XML이나 `@EnableTransactionManagement` + 수동 설정으로 AOP를 구성
- 최근엔 대부분 `@Transactional` 기반 + 자동 설정으로 충분

#### 프로그래밍 방식 트랜잭션 (TransactionTemplate, PlatformTransactionManager)
...
#### Controller/Repository 수준에서의 트랜잭션은 지양
...
- **로그 저장, 감사 기록, 알림 발송 등** "메인 트랜잭션과는 독립적인 부가 작업"에 `REQUIRES_NEW`를 쓰기도 한다.

### 자주 하는 실수와 주의점

#### 같은 클래스 내부의 메서드 호출 (self-invocation) 문제
...
- 같은 클래스 내부에서 `this.inner()` 호출은 프록시를 거치지 않기 때문에 `inner()`의 `@Transactional`이 무시된다.
...
#### 트랜잭션 안에서만 동작하는 JPA 기능을 트랜잭션 밖에서 사용
...
- **선언적 트랜잭션(@Transactional)** 을 기본으로 사용하고, 특별한 경우에만 프로그래밍 방식 트랜잭션을 고려한다.
...
이 원칙만 지켜도, 대부분의 JPA/Spring 기반 애플리케이션에서 트랜잭션 관련 버그와 데이터 일관성 문제를 크게 줄일 수 있다.

## 트레이드오프

해당 없음 — Propagation 옵션별 동작 차이는 위 특징/상세 참고.
...
원본 학습 노트(TIL)에 인용 링크가 없음. 원본에 로컬 위키 경로(`../../../TIL.wiki/database-connection-and-transaction-management.md`)로의 연결된 정리본 참조가 있었으나, 이 볼트 외부 경로라 링크로 옮기지 않음. 필요 시 원본 TIL 위키에서 직접 확인.
