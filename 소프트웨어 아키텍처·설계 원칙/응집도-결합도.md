...
> 태그: `#oop` `#design-principles` `#cohesion` `#coupling`<br>
> 작성일: 2025-11-01<br>
> 최종 수정일: 2026-06-25

## 정의

응집도(Cohesion)는 모듈 내부 요소들이 얼마나 밀접하게 관련되어 있는가, 즉 하나의 모듈이 단일한 목적을 얼마나 잘 수행하느냐를 나타내는 척도이고, 결합도(Coupling)는 모듈 간 의존성 정도, 즉 하나의 모듈이 다른 모듈에 얼마나 의존하느냐를 나타내는 척도다. 좋은 설계는 응집도는 높이고(High Cohesion) 결합도는 낮추는(Low Coupling) 방향으로 한다.
...
해당 없음 — 모듈/클래스 설계 원칙에 대한 개념 정리이며 분산 시스템 트레이드오프(일관성/가용성/지연/비용/운영부담)와는 무관. 응집도-결합도 수준별 유지보수성/변경 영향 범위는 위 특징/상세의 응집도·결합도 분류표와 예시 참고.

## 실무 경험

해당 없음 (관련 실무 내용은 특징/상세의 각 응집도/결합도 수준별 예시 — 특히 "기능적 응집"과 "데이터 결합"이 실무 권장 패턴) 참고.

## 참고

- 원본 학습 노트(TIL)에 `> 연결된 정리본: 응집도와 결합도` 링크(`../../../TIL.wiki/software-design-cohesion-and-coupling.md`)가 있었으나, 이 볼트 외부의 위키 경로라 마크다운 링크로 옮기지 않음. 필요 시 원본 TIL 위키에서 직접 확인.
