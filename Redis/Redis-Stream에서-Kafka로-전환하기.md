...
> 태그: `#redis` `#kafka` `#stream` `#migration`<br>
> 작성일: 2026-06-25<br>
> 최종 수정일: 2026-06-25

## 정의

Redis Streams는 메모리 기반 "빠른 큐"이고 Kafka는 디스크 기반 "영속 로그 + 이벤트 스트리밍 플랫폼"으로, 두 시스템을 동일한 인터페이스로 감싸면 ordering·재처리·delivery 보장 같은 실행 의미(semantics)가 어긋난다.
...
> Redis Streams 기초와 PEL 개념은 원본 TIL의 "Redis Stream으로 이벤트 로그 다루기" 노트를 참고한다 (아래 참고 섹션의 미해결 링크 안내 참조).
...
> DLQ, retry, dead letter 개념은 원본 TIL의 "메시징 패턴: MQ, Pub/Sub, Stream, Kafka" 노트를 참고한다 (아래 참고 섹션의 미해결 링크 안내 참조).
...
| 항목 | Redis Streams | Kafka |
|---|---|---|
| 일관성 | 메시지 단위 ack, ordering은 stream 전체에 느슨하게 적용 | key 단위 partition 내부에서만 ordering 보장, 일관성 모델이 더 명시적 |
| 가용성 | 단일 인스턴스 의존 시 장애에 취약 (클러스터 구성 시 완화) | 파티션 복제 기반으로 브로커 장애에 강함 |
| 지연 | in-memory라 지연 낮음 | 디스크 append-only라 Redis보다 지연 높음 (배치/복제로 추가 증가 가능) |
| 비용 | 메모리 비용, 데이터 보존량에 한계 | 디스크/클러스터 운영 비용, 다만 대용량 보존에는 유리 |
| 운영부담 | 단순하지만 메시지 유실 위험을 직접 관리해야 함 | partition 설계, rebalance, consumer offset 관리 등 운영 복잡도 높음 |

## 실무 경험

해당 없음 (관련 실무 내용은 특징/상세 5장 "점진적 마이그레이션 전략"에 포함)

## 참고

원본 학습 노트(TIL)에 다음 링크들이 있었으나 그대로 옮기지 않았다:

- `> 연결된 정리본: 메시징 패턴: MQ, Pub/Sub, Stream, Kafka` → `../../../TIL.wiki/messaging-patterns-pubsub-stream-kafka.md` — 이 볼트 외부의 위키 경로라 마크다운 링크로 옮기지 않음. 특징/상세 3-4장에도 동일 노트를 가리키는 인용이 있었음. 필요 시 원본 TIL 위키에서 직접 확인.
- 특징/상세 1-1장의 "[Redis Stream으로 이벤트 로그 다루기](<../Tech Blog/Redis Stream으로 이벤트 로그 다루기.md>)" 링크 — 원본이 가리키는 `../Tech Blog/Redis Stream으로 이벤트 로그 다루기.md` 파일이 현재 TIL 폴더 구조 어디에도 존재하지 않음(확인: 2026-06-25). 삭제 또는 경로 변경된 것으로 추정되며, 원본 작성자에게 확인이 필요한 깨진 참조다. 추측으로 내용을 채우지 않고 사실로 기록하지 않음.
