...
> 태그: `#redis` `#zset` `#ranking`<br>
> 작성일: 2026-06-25<br>
> 최종 수정일: 2026-06-25

## 정의

`ZSET`은 점수(score)를 기준으로 항상 정렬된 상태를 유지하는 자료구조로, 삽입 시점에 이미 정렬이 끝나 있어 조회 시 별도 정렬 없이 `O(log N + M)`으로 랭킹을 처리한다.
...
해당 없음 (특징/상세 8장 "자료구조 선택 기준 정리" 표 참고)

## 실무 경험

해당 없음 (관련 실무 내용은 특징/상세 4장 "실시간 업데이트 패턴", 5장 "확장 패턴"에 포함)

## 참고

원본 학습 노트(TIL)에 `> 연결된 정리본: Redis 개요와 캐싱` 링크(`../../../TIL.wiki/redis-overview-and-caching.md`)가 있었으나, 이 볼트 외부의 위키 경로라 마크다운 링크로 옮기지 않음. 필요 시 원본 TIL 위키에서 직접 확인.
