...
> 태그: `#websocket` `#sse` `#realtime` `#스케일링`<br>
> 작성일: 2026-06-25<br>
> 최종 수정일: 2026-06-25

## 정의

`WebSocket`은 HTTP 핸드셰이크 후 별도 프로토콜로 업그레이드해 클라이언트 ↔ 서버 완전 양방향(Full-Duplex) 통신을 제공하는 기술이고, `SSE(Server-Sent Events)`는 표준 HTTP 위에서 서버 → 클라이언트 단방향 푸시를 제공하는 기술이다 — 실시간 양방향 상호작용이 필요하면 WebSocket, 서버 푸시만 필요하면 SSE를 선택한다.
...
| 항목 | WebSocket | SSE |
|---|---|---|
| 일관성 | 단일 연결 내 순서 보장, 여러 서버로 확장 시 Redis Pub/Sub 등으로 브로드캐스트 동기화 필요 | Last-Event-ID 기반으로 재연결 시 누락 이벤트 보완 가능 |
| 가용성 | 세션 고정(Sticky Session) 또는 메시지 브로커 필요 — 서버 장애 시 해당 서버 연결 전체 끊김 | 브라우저가 자동 재연결 처리 — 안정성 높음 |
| 지연 | 헤더 오버헤드가 적어 메시지 즉시 전송, 양방향 상호작용에 유리 | HTTP 기반이라 WebSocket보다 약간 높을 수 있음 |
| 비용 | 지속 연결 유지로 서버 메모리/소켓 비용 증가 (C10K 고려) | 브라우저당 6개 연결 제한(HTTP/1.1) — HTTP/2 또는 서브도메인 분산 필요 |
| 운영부담 | 프록시/로드밸런서 WebSocket 설정, Sticky Session, 하트비트 직접 구현 필요 — 운영 복잡도 높음 | HTTP 인프라 재사용 가능, 구현·운영이 상대적으로 단순 |

## 실무 경험

해당 없음 (관련 실무 내용은 특징/상세 4장 "WebSocket 구현", 5장 "SSE 구현", 6장 "인프라 및 스케일링 고려사항"에 포함)

## 참고

원본 학습 노트(TIL)에 `> 연결된 정리본: WebSocket과 SSE` 링크(`../../../TIL.wiki/websocket-and-sse.md`)가 있었으나, 이 볼트 외부의 위키 경로라 마크다운 링크로 옮기지 않음. 필요 시 원본 TIL 위키에서 직접 확인.
