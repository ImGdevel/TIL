...
> 태그: `#websocket` `#sse` `#realtime`<br>
> 작성일: 2026-06-25<br>
> 최종 수정일: 2026-06-25

## 정의

`SSE`는 서버 → 클라이언트 단방향 스트리밍을 HTTP 위에서 그대로 구현하는 기술이고, `WebSocket`은 HTTP 핸드셰이크 후 별도 프로토콜로 업그레이드해 서버 ↔ 클라이언트 양방향 통신을 제공하는 기술이다.
...
> 원본 TIL 노트에는 이 섹션과 다음 "장단점 비교" 섹션이 둘 다 "6."으로 중복 표기되어 있었다. 내용 손실 없이 이 노트에서는 6장(코드 예제), 7장(장단점 비교)으로 순서를 바로잡아 기록한다.
...
| 항목 | SSE | WebSocket |
|---|---|---|
| 일관성 | Last-Event-ID로 재연결 시 누락 이벤트 보완 가능 | 메시지 순서는 단일 연결 내 보장, 다중 서버 확장 시 브로커 동기화 필요 |
| 가용성 | 브라우저가 자동 재연결 처리 — 안정성 높음 | 재연결 로직을 직접 구현해야 하며, 서버 장애 시 연결 전체 끊김 |
| 지연 | HTTP 기반이라 WebSocket보다 약간 높을 수 있음 | 헤더 오버헤드가 적어 짧은 메시지 빈번 교환에 더 유리 |
| 비용 | 브라우저당 6개 연결 제한(HTTP/1.1) — HTTP/2 또는 서브도메인 분산 필요 | 지속 연결 유지로 서버 메모리/소켓 비용 증가 |
| 운영부담 | 구현 간단, 기존 HTTP 인프라 재사용 가능 | 세션 스티키니스, 메시지 브로커, 프록시 설정 등 운영 복잡도 높음 |

## 실무 경험

해당 없음 (관련 실무 내용은 특징/상세 6장 "Spring 코드 예제", 7장 "장단점 비교"에 포함)
