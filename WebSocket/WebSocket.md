...
> 태그: `#websocket` `#realtime` `#stomp`<br>
> 작성일: 2026-06-25<br>
> 최종 수정일: 2026-06-25

## 정의

`WebSocket`은 브라우저와 서버가 HTTP로 핸드셰이크한 뒤 하나의 TCP 연결 위에서 양방향(full-duplex) 통신으로 업그레이드되는 프로토콜로, 헤더 오버헤드가 적어 짧은 메시지를 자주 주고받는 실시간 상호작용에 적합하다.
...
| 항목 | 내용 |
|---|---|
| 일관성 | 단일 연결 내 메시지 순서는 보장되나, 다중 서버로 확장 시 메시지 브로커로 상태를 동기화해야 함 |
| 가용성 | 서버 인스턴스 장애 시 해당 서버의 모든 연결이 끊김 — 세션 스티키니스/세션 공유로 완화 필요 |
| 지연 | HTTP 요청/응답 반복보다 낮음 — 헤더 오버헤드가 적어 짧은 메시지 빈번 교환에 유리 |
| 비용 | 지속 연결 유지로 서버 메모리/소켓 리소스 비용 증가 (C10K급 동시 연결 고려) |
| 운영부담 | 재연결, 하트비트, 인증 재검증, 로드밸런서/프록시 WebSocket 설정을 직접 구현·운영해야 함 |

## 실무 경험

해당 없음 (관련 실무 내용은 특징/상세 6장 "장점", 7장 "단점·주의사항"에 포함)
